 ...
 *  Flexible schema, fully reorganized, no SQLite code remaining.
 *  Includes: DB, middleware, auth, posts, comments, messages,
 *  reactions, notifications, socket.io, utilities in one file.
 ********************************************************************/
...
async function createPoolOrExit() {
  ...
  // normalize short hostnames and validate DNS before creating the pool
  try {
    const dns = require("dns").promises;
    const urlObj = new URL(conn);

    // convenience: append render.com if hostname looks truncated (no dot)
